<x-app-layout>
    <x-slot name="header">
        <div class="relative">
            ...
            <h2 class="mt-1 text-2xl font-black tracking-tight text-neutral-950">Daftarkan Kamar Baru</h2>
            <p class="mt-1 text-sm font-medium text-neutral-500">Buat tipe unit penginapan baru untuk dipublikasikan langsung ke situs web.</p>
        </div>
    </x-slot>
    ...
    <div class="relative min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,0.06),transparent_40rem)] py-10">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('rooms.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @include('rooms._form', ['submitLabel' => 'Daftarkan Kamar'])
            </form>
        </div>
    </div>
</x-app-layout>
